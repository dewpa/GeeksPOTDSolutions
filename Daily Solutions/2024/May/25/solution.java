...
class Solution {
    long max_Books(int arr[], int n, int k) {
        ...
        long currentSum = 0; // To store the sum of books in the current window
        long maxBooksCollected = 0; // To store the maximum number of books collected
        ...
        while (right < n) {
            ...
            if (arr[right] <= k) {
                ...
                maxBooksCollected = Math.max(maxBooksCollected, currentSum);
